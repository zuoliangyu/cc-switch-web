...
現在のリポジトリバージョンは `0.5.1` です。

`0.5.1` は挙動変更を伴わないエンジニアリング修正リリースで、vitest の失敗数を 0.4.0 基準の 26 件から **0 件**へ収束させました（184 passed + 2 skipped = 186/186）。

- 型 / 形状の追従漏れ：`McpFormModal` の apps、`useDirectorySettings.resolvedDirs`、`useSettings.resetAllDirectories` の引数列に `hermes` / `openclaw` を追加。
- API 改名 / プロトコル変更：`useInstallSkillArchives`（旧 `useInstallSkillsFromZip`）、`useImportExport` テスト群を Web hook の新 API に合わせて書き直し（`selectImportUpload(File)`、`importConfigFromUpload`、`{blob, fileName}` を返す `downloadConfigExport`）、`updateProvider` の payload 形状、ローカル Web ポート `8788 → 8890`。
- UI / 挙動の差分：仮想化リストは `getByRole("heading")` で検証、「已选 N 项」が複数箇所に出るため `getAllByText` に変更、`EditProviderDialog onSubmit` を `{ provider, originalId }` 形に揃え、`useTranslation` を mock して i18n 参照を固定（初期化 `useEffect` が再実行されて `resetSettings` を上書きする問題を回避）、App 統合テストに `retry: 2`。
- Web モードで存在しなくなった 2 件は `it.skip`：`SettingsDialog` のネイティブファイルダイアログ経路、ディレクトリ参照ボタン経路（Web にはネイティブピッカーがなく、等価な振る舞いは `useImportExport` / `useDirectorySettings` の単体テストでカバー済み）。

バックエンド `cargo test --lib --test-threads=1` は引き続き 775/775 全パス、フロントエンド `pnpm tsc` は 0 エラー。
