...
現在のリポジトリバージョンは `2.2.0` です。本バージョンでは、MiniMax Code アプリとロール別の Claude モデルマッピングを追加し、セッション使用量の増分スキャンとサブスクリプション使用量、共有ワークスペースにおける Codex OAuth アカウント分離を実装しました。あわせて最新のアップストリーム Provider プリセットとプロキシプロトコル修正を同期しています。リリース詳細とアップストリーム移行台帳は `CHANGELOG.md` および `docs-dev/web-parity-post-fd14f9c4-2026-09.md` を参照してください。
