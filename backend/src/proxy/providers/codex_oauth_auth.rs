...
impl CodexOAuthManager {
    ...
    /// Web 端测试辅助：写入或覆盖一个托管账号（同 workspace id），刷新 token 世代递增。
    #[cfg(test)]
    pub(crate) async fn seed_account_for_test(
        ...
        accounts.insert(
            ...
            CodexAccountData {
                ...
                // 旧测试传入占位 id_token；账号身份校验需要带 sub 的 JWT。
                id_token: Some(if id_token.contains('.') {
                    id_token.to_string()
                } else {
                    crate::codex_config::test_codex_id_token(id_token)
                }),
