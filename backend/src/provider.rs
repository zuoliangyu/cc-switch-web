...
/// Codex Responses -> Chat Completions 的 reasoning 能力描述（跟随上游 44d9aabb）。
...
pub struct CodexChatReasoningConfig {
    ...
    /// 靠穷举字段提取、并不读取本字段；保留作文档说明与未来按格式分发的预留。
...
pub struct ProviderMeta {
    ...
    /// Codex Responses -> Chat Completions 的 reasoning 能力元数据（跟随上游 44d9aabb）。
