...
/// 后台会话用量扫描间隔（与上游 cc-switch 桌面端一致）。
const SESSION_SYNC_INTERVAL_SECS: u64 = 60;

/// 会话用量是否处于自动扫描模式（设置 `sessionAutoSyncEnabled`，默认开启）。
///
/// 只管会话日志的扫描时机：代理接管记账是实时的，从不读会话文件，不受影响。
pub fn session_auto_sync_enabled() -> bool {
    crate::settings::get_settings().session_auto_sync_enabled
}

/// 启动后台会话用量扫描（上游 cc-switch 092ea1f3 的 Web 适配）。
///
/// 自动模式下启动即扫描一轮，此后每 [`SESSION_SYNC_INTERVAL_SECS`] 秒扫描一次；
/// 手动模式下每轮直接跳过（含首轮），仅由用量面板的"立即同步"触发扫描。
/// 每轮实时读取设置，切换模式无需重启服务。
pub(crate) fn start_background_sync_worker(db: std::sync::Arc<Database>) {
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            run_background_sync_round(db.clone()).await;
        }
    });
}

async fn run_background_sync_round(db: std::sync::Arc<Database>) {
    if !session_auto_sync_enabled() {
        return;
    }
    let _guard = session_sync_mutex().lock().await;
    // 持锁后再确认一次：等待手动同步期间用户可能已切到手动模式
    if !session_auto_sync_enabled() {
        return;
    }
    match tokio::task::spawn_blocking(move || sync_all_unlocked(&db)).await {
        Ok(result) => {
            for error in &result.errors {
                log::debug!("[SESSION-SYNC] 后台扫描: {error}");
            }
        }
        Err(error) => log::warn!("[SESSION-SYNC] 后台扫描任务失败: {error}"),
    }
}
