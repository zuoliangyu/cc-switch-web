...
/// 否则 Claude Code 模型菜单会残留上一个供应商的显示名称（上游 f93b935d）。
...
impl ProxyService {
    ...
    /// 接管期间切换 Claude 供应商：按目标供应商重写 Live，模型字段写成稳定角色
    /// 别名 + 真实显示名，使 Claude Code 模型菜单跟随当前供应商（上游 f93b935d）。
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            ...
            self.set_takeover_enabled_for_app(app_type_str, true)
                .await?;
            return Ok(());
        }
    ...
    async fn disable_takeover_for_app_inner(&self, app: &AppType) -> Result<(), String> {
        let app_type_str = app.as_str();
        ...
        self.set_takeover_enabled_for_app(app_type_str, false)
            .await?;
                        ...
                        self.db.get_provider_by_id(&provider_id, "grokbuild")
                    {
                            ...
                            .filter(|token| !token.is_empty() && token != PROXY_TOKEN_PLACEHOLDER)
                        {
                            ...
                            self.db
                                ...
                                .map_err(|e| format!("同步 Grok Build Token 到数据库失败: {e}"))?;
                        }
                    }
                }
            }
    ...
    fn apply_claude_takeover_fields_for_provider(
        ...
        let takeover_model_fields = if provider.uses_managed_account_auth() {
            ...
            Self::build_claude_takeover_model_fields(live_config)
        };
    ...
    fn apply_claude_takeover_fields_with_policy(
        ...
        let takeover_model_fields = Self::build_claude_takeover_model_fields(live_config);
    ...
    /// - 移除模型覆盖 env（CLAUDE_MODEL_OVERRIDE_ENV_KEYS），再写入稳定角色别名与显示名
    ...
    fn apply_claude_takeover_fields_with_policy_and_models(
        ...
        if !live_config.get("env").is_some_and(Value::is_object) {
        ...
        if let Some(env) = live_config.get_mut("env").and_then(|v| v.as_object_mut()) {
            ...
            // 移除旧模型覆盖字段后写入稳定角色别名，避免切换供应商后仍用旧模型名发起请求
    ...
    async fn restore_live_config_for_app_with_fallback_inner(
        ...
        if let Some(backup) = backup {
            ...
            if Self::live_has_proxy_placeholder_for_app(app_type, &config) {
                ...
                let managed_codex_auth = if matches!(app_type, AppType::Codex) {
    ...
    pub fn detect_takeover_in_live_config_for_app(&self, app_type: &AppType) -> bool {
        match app_type {
            ...
            AppType::Hermes | AppType::Pi | AppType::Mcode => false,
        }
    }
    ...
    fn cleanup_takeover_placeholders_in_live_for_app(
        ...
        match app_type {
            ...
            AppType::Hermes | AppType::Pi | AppType::Mcode => Ok(()),
        }
    }
    ...
    async fn apply_managed_codex_auth_to_restore(
        ...
        let bundle = manager
            ...
            .await
            .map_err(|error| format!("Codex OAuth 账号不可用: {error}"))?;
        if bundle.id_token.is_none() {
            return Err(format!(
                "Codex OAuth 账号 {account_id} 缺少 id_token，请重新登录"
            ));
        }
    ...
    pub async fn switch_proxy_target(
        ...
        let _guard = self.switch_locks.lock_for_app(app_type_enum.as_str()).await;
        ...
        if has_backup || live_taken_over {
            Self::update_live_backup_from_provider_for_db(self.db.as_ref(), app_type, &provider)
                .await?;
...
",
    ...
    #[tokio::test]
    #[serial]
    async fn codex_managed_restore_injects_latest_token_without_storing_it_in_backup() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        let db = Arc::new(Database::memory().expect("init db"));
        let service = ProxyService::new(db.clone());
        ...
        assert!(!backup.original_config.contains("latest-refresh"));
        assert!(!backup.original_config.contains("latest-access"));
        ...
        let live = service.read_codex_live().unwrap();
        assert_eq!(live["auth"]["tokens"]["access_token"], "latest-access");
        assert_eq!(live["auth"]["tokens"]["refresh_token"], "latest-refresh");
        assert_eq!(live["auth"]["tokens"]["account_id"], "account-one");
        assert!(
    ...
    // 以下接管模型别名测试移植自上游 f93b935d / b3e5e32c / 4b57f7e1。
    #[test]
    ...
    #[test]
    fn normal_claude_takeover_without_env_writes_placeholder_and_base_url() {
        let mut live_config = json!({});
        ...
        let env = live_config["env"].as_object().expect("env");
        assert_env_str(env, "ANTHROPIC_BASE_URL", Some("http://127.0.0.1:15721"));
        assert_env_str(env, "ANTHROPIC_AUTH_TOKEN", Some(PROXY_TOKEN_PLACEHOLDER));
    }

    /// 上游 f93b935d：接管期间切换 Claude 供应商时按目标供应商重写 Live 的模型别名与显示名。
    #[tokio::test]
    #[serial]
    async fn switch_proxy_target_updates_claude_live_model_aliases() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        ...
        let env = live["env"].as_object().expect("live env");
        ...
        let restored: Value = serde_json::from_str(&backup.original_config).expect("backup json");
        assert_eq!(restored["env"]["ANTHROPIC_API_KEY"], "b-key");
    }
