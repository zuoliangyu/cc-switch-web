...
pub fn proxy_gateway_base_url_from_db(db: &Database) -> Result<String, AppError> {
    // get_global_proxy_config is async-tagged but its body is fully synchronous
    // (rusqlite under a Mutex), so block_on cannot deadlock the calling thread.
    let config = futures::executor::block_on(db.get_global_proxy_config())?;
    Ok(format!(
        "{}{}",
